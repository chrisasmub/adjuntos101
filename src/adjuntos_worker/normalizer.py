...
def normalize_document(
    ...
    issue_date = _extract_date(text, ["issue date", "fecha emision", "fecha de emision", "fecha"])
    due_date = _extract_date(text, ["due date", "fecha vencimiento", "vencimiento"])
    ...
    total_amount = _extract_amount(text, ["total amount", "monto total", "total", "importe total"])
...
def _extract_labeled_text(text: str, labels) -> str:
    for label in labels:
        pattern = re.compile(r"{0}\s*[:#-]\s*(.+)".format(re.escape(label)), re.IGNORECASE)
...
def _extract_date(text: str, labels) -> Optional[date]:
    for label in labels:
        pattern = re.compile(
            r"{0}\s*[:#-]\s*([0-9]{{4}}-[0-9]{{2}}-[0-9]{{2}}|[0-9]{{2}}/[0-9]{{2}}/[0-9]{{4}}|[0-9]{{2}}-[0-9]{{2}}-[0-9]{{4}})".format(
                re.escape(label)
            ),
            re.IGNORECASE,
        )
...
def _extract_amount(text: str, labels) -> Optional[float]:
    for label in labels:
        pattern = re.compile(
            r"{0}\s*[:#-]?\s*(?:CLP|USD|UF|EUR|\$)?\s*([0-9][0-9\.\,]*)".format(re.escape(label)),
            re.IGNORECASE,
        )
        match = pattern.search(text)
        if match:
